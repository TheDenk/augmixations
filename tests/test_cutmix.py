...
@pytest.mark.parametrize('params', 
    [(np.ones((500, 500, 3), dtype=np.uint8)*255, 0, 0, 500, 500, 100, 100, 300, 300), 
    (np.ones((1000, 2000, 3), dtype=np.uint8)*255, 0, 0, 2000, 1000, 300, 300, 600, 600), 
    (np.ones((500, 1000, 3), dtype=np.uint8)*255, 0, 0, 500, 500, None, None, None, None), 
    (np.ones((500, 700, 3), dtype=np.uint8)*255, None, None, None, None, None, None, None, None), 
    (np.ones((300, 300, 3), dtype=np.uint8)*255, None, None, None, None, 100, 100, 300, 300)]) 
def test_generate_rect_coordinates(params):
    fg_image, min_x, min_y, max_x, max_y, min_h, min_w, max_h, max_w = params
    rect = generate_rect_coordinates(fg_image, min_x, min_y, max_x, max_y, min_h, min_w, max_h, max_w)
    x1, y1, x2, y2 = rect

    assert x1 < x2 and y1 < y2
    assert x1 >= 0 and y1 >= 0
    assert x2 <= fg_image.shape[1] and y2 <= fg_image.shape[0]
...
def test_insert_image_in_background(params):
    bg_img, fg_img, start_crop_position, min_x, min_y, max_x, max_y = params

    x, y = start_crop_position
    out_img, (shift_x, shift_y) = insert_image_in_background(bg_img, fg_img, start_crop_position, min_x, min_y, max_x, max_y)

    if max_x is not None:
        assert x + shift_x <= max_x 
    if min_x is not None:
        assert x + shift_x >= min_x
    if max_y is not None:
        assert y + shift_y <= max_y 
    if min_y is not None:
        assert y + shift_y >= min_y

    assert out_img.shape == bg_img.shape


@pytest.mark.parametrize('params', 
                        [(np.array([0, 0, 200, 300]), np.array([np.array([10, 10, 50, 50]), 
                          np.array([20, 10, 60, 40])]), 50, 100, np.array([50, 100, 250, 400]), 
                          np.array([np.array([60, 110, 100, 150]), np.array([70, 110, 110, 140])]))]) 
...
@pytest.mark.parametrize('params', \
           ...
           [({'x1':75, 'y1':25, 'x2':125, 'y2':125, 'area':50*100, 'height':100, 'width':50}, 
            {'x1':50, 'y1':50, 'x2':150, 'y2':150, 'area':100*100, 'height':100, 'width':100}, 0.3, True), 
            ({'x1':75, 'y1':25, 'x2':125, 'y2':125, 'area':50*100, 'height':100, 'width':50}, 
            {'x1':50, 'y1':50, 'x2':150, 'y2':150, 'area':100*100, 'height':100, 'width':100}, 0.4, False), 
...
@pytest.mark.parametrize('params', [
            ...
            {'x1':50, 'y1':50, 'x2':150, 'y2':150, 'area':100*100, 'height':100, 'width':100}, 
            0.1, 0.1, 0.7, np.array([50, 50, 150, 150]), False),

            ({'x1':25, 'y1':25, 'x2':125, 'y2':125, 'area':100*100, 'height':100, 'width':100}, 
            {'x1':50, 'y1':50, 'x2':150, 'y2':150, 'area':100*100, 'height':100, 'width':100}, 
            0.1, 0.1, 0.5, np.array([50, 50, 150, 150]), True),

            ({'x1':25, 'y1':25, 'x2':125, 'y2':125, 'area':100*100, 'height':100, 'width':100}, 
            {'x1':50, 'y1':50, 'x2':150, 'y2':150, 'area':100*100, 'height':100, 'width':100}, 
            0.3, 0.1, 0.7, np.array([125, 50, 150, 150]), False),

            ({'x1':25, 'y1':25, 'x2':125, 'y2':125, 'area':100*100, 'height':100, 'width':100}, 
            {'x1':50, 'y1':50, 'x2':150, 'y2':150, 'area':100*100, 'height':100, 'width':100}, 
            0.1, 0.3, 0.7, np.array([50, 125, 150, 150]), False),

            ({'x1':25, 'y1':25, 'x2':125, 'y2':125, 'area':100*100, 'height':100, 'width':100}, 
            {'x1':50, 'y1':50, 'x2':150, 'y2':150, 'area':100*100, 'height':100, 'width':100}, 
            0.3, 0.3, 0.5, np.array([125, 125, 150, 150]), True),
            ...
            ({'x1':75, 'y1':25, 'x2':175, 'y2':125, 'area':100*100, 'height':100, 'width':100}, 
            {'x1':50, 'y1':50, 'x2':150, 'y2':150, 'area':100*100, 'height':100, 'width':100}, 
            0.3, 0.1, 0.7, np.array([50, 50, 75, 150]), False),
            ...
            ({'x1':75, 'y1':25, 'x2':175, 'y2':125, 'area':100*100, 'height':100, 'width':100}, 
            {'x1':50, 'y1':50, 'x2':150, 'y2':150, 'area':100*100, 'height':100, 'width':100}, 
            0.3, 0.3, 0.5, np.array([50, 125,  75, 150]), True),
            ...
            ({'x1':25, 'y1':75, 'x2':125, 'y2':175, 'area':100*100, 'height':100, 'width':100}, 
            {'x1':50, 'y1':50, 'x2':150, 'y2':150, 'area':100*100, 'height':100, 'width':100}, 
            0.3, 0.1, 0.7, np.array([50, 125, 150, 150]), False),

            ({'x1':25, 'y1':75, 'x2':125, 'y2':175, 'area':100*100, 'height':100, 'width':100}, 
            {'x1':50, 'y1':50, 'x2':150, 'y2':150, 'area':100*100, 'height':100, 'width':100}, 
            0.1, 0.3, 0.7, np.array([50,  50, 150,  75]), False),

            ({'x1':25, 'y1':75, 'x2':125, 'y2':175, 'area':100*100, 'height':100, 'width':100}, 
            {'x1':50, 'y1':50, 'x2':150, 'y2':150, 'area':100*100, 'height':100, 'width':100}, 
            0.3, 0.3, 0.5, np.array([50, 125, 150, 75]), True),

            ## ПРАВЫЙ НИЖНИЙ УГОЛ
            ({'x1':100, 'y1':100, 'x2':200, 'y2':200, 'area':100*100, 'height':100, 'width':100}, 
            {'x1':50, 'y1':50, 'x2':150, 'y2':150, 'area':100*100, 'height':100, 'width':100}, 
            0.1, 0.1, 0.5, np.array([50, 50, 150, 150]), False),

            ({'x1':100, 'y1':100, 'x2':200, 'y2':200, 'area':100*100, 'height':100, 'width':100}, 
            {'x1':50, 'y1':50, 'x2':150, 'y2':150, 'area':100*100, 'height':100, 'width':100}, 
            0.1, 0.1, 0.2, np.array([50, 50, 150, 150]), True),

            ({'x1':100, 'y1':100, 'x2':200, 'y2':200, 'area':100*100, 'height':100, 'width':100}, 
            {'x1':50, 'y1':50, 'x2':150, 'y2':150, 'area':100*100, 'height':100, 'width':100}, 
            0.6, 0.1, 0.7, np.array([50, 50, 100, 150]), False),

            ({'x1':100, 'y1':100, 'x2':200, 'y2':200, 'area':100*100, 'height':100, 'width':100}, 
            {'x1':50, 'y1':50, 'x2':150, 'y2':150, 'area':100*100, 'height':100, 'width':100}, 
            0.1, 0.6, 0.7, np.array([50,  50, 150,  100]), False),

            ({'x1':100, 'y1':100, 'x2':200, 'y2':200, 'area':100*100, 'height':100, 'width':100}, 
            {'x1':50, 'y1':50, 'x2':150, 'y2':150, 'area':100*100, 'height':100, 'width':100}, 
            0.6, 0.6, 0.2, np.array([50, 50, 100, 100]), True),
])
def test_correct_box_if_some_alnge_overlap(params):
    ...
    new_box, critical_overlap = correct_box_if_some_alnge_overlap(rect_info, box_info, min_h_overlap, min_w_overlap, max_overlap_area)
    assert real_overlap == critical_overlap
    assert np.array_equal(true_box, new_box)
...
@pytest.mark.parametrize('params', [
            ...
            np.array([100, 100, 200, 200]), 0.5, 0.25, 0.25, 0.1, 0.1,
            np.array([np.array([50, 50, 150, 150])]), np.array(['1'], dtype=object)),
            ...
            (np.array([np.array([50, 50, 150, 150])]), np.array(['1'], dtype=object),
            np.array([100, 100, 200, 200]), 0.2, 0.25, 0.25, 0.1, 0.1,
            np.array([]), np.array([])),
            ...
            # с помощью увеличение минимального коэфта мин ширины до 0.8
            (np.array([np.array([50, 50, 150, 150])]), np.array(['1'], dtype=object),
            np.array([100, 100, 200, 200]), 0.8, 0.7, 0.25, 0.1, 0.8,
            np.array([]), np.array([])),
            ...
            # с помощью увеличение минимального коэфта мин высоты до 0.8
            (np.array([np.array([50, 50, 150, 150])]), np.array(['1'], dtype=object),
            np.array([100, 100, 200, 200]), 0.8, 0.25, 0.7, 0.8, 0.1,
            np.array([]), np.array([])),
...
def test_correct_background_boxes(params):
    bg_boxes, bg_labels, image_rect, max_overlap, min_h, min_w, min_h_overlap, min_w_overlap, real_boxes, real_labels = params
    boxes, labels = correct_background_boxes(bg_boxes, bg_labels, image_rect, max_overlap, min_h, min_w, min_h_overlap, min_w_overlap)
...
@pytest.mark.parametrize('params', [
    (np.ones((1000, 2000, 3), dtype=np.uint8)*255, np.array([np.array([50, 50, 150, 150])]), np.array(['1'], dtype=np.str),
     np.ones((600, 800, 3), dtype=np.uint8)*255, np.array([np.array([100, 100, 200, 200])]), np.array(['2'], dtype=np.str),
     {'crop_min_x': 75, 'crop_max_x':76, 'crop_min_y':75, 'crop_max_y':76, 'min_rect_h' : 150, 
      ...
      np.array([np.array([50, 50, 150, 150]), np.array([125, 125, 225, 225])]), np.array(['1', '2'], dtype=np.str)
    )
])
def test_cutmix(params):
    bg_img, bg_boxes, bg_labels, fg_img, fg_boxes, fg_labels, crop_rect_config, process_boxes_config, real_boxes, real_labels = params
